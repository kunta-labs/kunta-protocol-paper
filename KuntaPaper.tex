...
\begin{quotation}
"There is no room for \(\texttt{\boldtf{Tribalism}}\), when the mission is democratization"
\end{quation}
...
\begin{enumerate}
	\item \texttt{\boldtf{SDMP}}: Single Design Multi-Purpose 
	\item \texttt{\boldtf{FDSP}}: Flexible Design Single Purpose
\end{enumerate}

A \texttt{\boldtf{SDMP}} is a protocol that is designed to be multipurpose and general, and the amount of "supported" computation types, and "acceptable" computation types tends towards \(\infty\). These protocol types have one specific design, but users are encouraged to use the protocol "as-is". 

With an \texttt{\boldtf{FDSP}} protocol, the "supported" computation types also tends towards \(\infty\), but is bounded above by the implementing developer. This means developers choose what computation are allowed to take place on the network.
...
A graph database is any storage system that provides \boldtf{index-free adjacency}. We use an ordered key-value store for root instance relationship storage, and propose its use for any relational data to be extracted from a chain. By using hexastores as the fundamental data store for each , we can further optimize relationship queries.
